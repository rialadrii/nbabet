...
def download_data():
    progress_text = "Descargando datos completos (incluyendo IDs de partido)..."
...
def load_data():
    ...
    if os.path.exists(csv_path):
        ...
        # Asegurar que game_id es string
        if 'game_id' in df.columns: 
            df['game_id'] = df['game_id'].astype(str).str.zfill(10)
        else:
            df['game_id'] = None # Evita error si la columna no existe
        return df
    return pd.DataFrame()
    ...
    if df.empty:
        ...
        if jugador:
            ...
            if 'game_id' in view.columns:
                view['FICHA'] = view['game_id'].apply(lambda x: f"<a href='https://www.nba.com/game/{x}' target='_blank' class='match-link'>📊 Ver</a>" if pd.notnull(x) else "<span class='no-link'>Actualiza Datos</span>")
                view = view.drop(columns=['game_id'])
                view = view[['game_date', 'matchup', 'FICHA', 'min', 'pts', 'reb', 'ast']] # Reordenar
            else:
                view['FICHA'] = "<span class='no-link'>Actualiza Datos</span>"
    ...
    if df.empty:
        ...
        if t1 and t2:
            ...
            for date in last_dates:
                ...
                link = f"<a href='https://www.nba.com/game/{g_id}' target='_blank' class='match-link'>📊 Ver Ficha</a>" if pd.notnull(g_id) else "<span class='no-link'>Falta ID</span>"
            ...
            if 'game_id' not in df.columns: st.warning("⚠️ Aviso: Faltan los enlaces. Ve a 'Actualizar Datos' para arreglarlo.")
